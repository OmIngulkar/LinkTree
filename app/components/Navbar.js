    ...
    <>
      {showNavbar && (
        <motion.nav
          ...
          className="bg-white/80 backdrop-blur-md shadow-md w-full md:w-[80vw] flex flex-col md:flex-row justify-between items-center fixed top-5 right-0 md:right-[10vw] border border-gray-300 rounded-xl md:rounded-full py-4 px-6 z-50"
        >
          {/* Left side (logo + links) */}
          <div className="w-full flex justify-between items-center md:w-auto">
            <Link href="/">
              <svg className="h-8 hover:scale-110 transition-transform" viewBox="0 0 1176 238" xmlns="http://www.w3.org/2000/svg">
            ...
            <button onClick={() => setIsOpen(!isOpen)} className="md:hidden text-xl font-bold">
              {isOpen ? '✖' : '☰'}
            </button>
          </div>

          {/* Desktop Links */}
          <ul className="hidden md:flex flex-wrap gap-6 text-gray-700 font-medium">
            <Link href="/"><li className="hover:text-green-800 transition">Templates</li></Link>
            <Link href="/"><li className="hover:text-green-800 transition">Marketplace</li></Link>
            <Link href="/"><li className="hover:text-green-800 transition">Discover</li></Link>
            <Link href="/"><li className="hover:text-green-800 transition">Pricing</li></Link>
            <Link href="/"><li className="hover:text-green-800 transition">Learn</li></Link>
          </ul>

          {/* Desktop Buttons */}
          <div className="hidden md:flex gap-3">
            <button className="bg-gray-300 hover:bg-gray-400 transition px-5 py-2 rounded-lg font-semibold text-gray-800">
              Log in
            </button>
            <button className="bg-black hover:bg-gray-800 transition px-6 py-2 rounded-full font-semibold text-white shadow-md">
              Sign up free
            </button>
          </div>
          ...
          {/* Mobile Dropdown */}
          {isOpen && (
            <ul className="md:hidden mt-8 w-full bg-blue-300 rounded-lg shadow p-4 text-gray-800 space-y-3">
              <Link href="/" onClick={() => setIsOpen(false)}><li>Templates</li></Link>
              <Link href="/" onClick={() => setIsOpen(false)}><li>Marketplace</li></Link>
              <Link href="/" onClick={() => setIsOpen(false)}><li>Discover</li></Link>
              <Link href="/" onClick={() => setIsOpen(false)}><li>Pricing</li></Link>
              <Link href="/" onClick={() => setIsOpen(false)}><li>Learn</li></Link>
              <li className="flex flex-col gap-2 pt-2">
                <button className="bg-gray-300 hover:bg-gray-400 px-4 py-2 rounded-md">Log in</button>
                <button className="bg-black text-white hover:bg-gray-900 px-4 py-2 rounded-md">Sign up free</button>
              </li>
            </ul>
          )}
        </motion.nav>
